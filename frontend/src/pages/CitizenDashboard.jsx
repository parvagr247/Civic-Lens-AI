...
import { Loader2, Award, Flame, CheckCircle, FileText, MapPin, Sparkles, Trophy, Calendar, Eye } from 'lucide-react';
      ...
      <Card className="p-6 bg-slate-900/30 border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <img 
            ...
            alt="Citizen Avatar" 
            className="w-16 h-16 rounded-2xl bg-slate-950/50 border border-slate-800 p-1 shrink-0 shadow-lg"
          />
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-black text-white">Welcome back, {data?.name || 'Citizen'}</h2>
              <span className="px-2 py-0.5 bg-emerald-950 text-emerald-400 border border-emerald-900 rounded text-[9px] font-black uppercase tracking-wider">
                {data?.level || 'New Citizen'}
              </span>
            </div>
            <p className="text-xs text-slate-400 leading-normal max-w-md">{data?.bio}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-4 bg-slate-900/30 border-slate-800 flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-xl font-black text-white">{data?.points || 0} XP</span>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-slate-900/30 border-slate-800 flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-xl font-black text-white">#{data?.rank || 1} Rank</span>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-slate-900/30 border-slate-800 flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-xl font-black text-white">{data?.reportsSubmitted || 0} Filed</span>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-slate-900/30 border-slate-800 flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-xl font-black text-white">{data?.reportsResolved || 0} Fixed</span>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-slate-900/20 border border-slate-800 rounded-xl p-5 shadow-lg space-y-4">
            ...
            {(!data?.recentReports || data.recentReports.length === 0) ? (
              <div className="text-center py-8 space-y-3">
                ...
                <Button 
                  onClick={() => navigate('/analyze')}
                  className="bg-slate-900 border border-slate-800 text-slate-200 text-xs hover:border-slate-700"
                >
                  Report First Issue
                </Button>
              </div>
              ...
              <div className="space-y-3">
                {data.recentReports.map(report => (
                  <div key={report.id} className="p-3.5 bg-slate-950/30 border border-slate-850 rounded-xl hover:border-slate-750 transition-all duration-200 flex items-center justify-between gap-4">
          ...
          <div className="bg-slate-900/20 border border-slate-800 rounded-xl p-5 shadow-lg space-y-4">
            ...
            <div className="flex items-end justify-between h-40 pt-4 px-2 bg-slate-950/20 rounded-xl border border-slate-900">
              {data?.weeklyActivity && Object.entries(data.weeklyActivity).map(([day, val]) => {
                ...
                return (
                  <div key={day} className="flex flex-col items-center flex-1 group">
                    <span className="text-[9px] font-bold text-slate-500 group-hover:text-emerald-400 transition-colors duration-200 mb-1">
                      {val}
                    </span>
                    <div 
                      className={`w-5 rounded-t-sm transition-all duration-500 ${
                        val > 0 ? 'bg-emerald-500/70 group-hover:bg-emerald-400 shadow-lg' : 'bg-slate-850'
                      }`}
                      style={{ height: `${Math.max(heightPercent, 4)}%` }}
                    />
                    <span className="text-[9px] font-semibold text-slate-450 mt-2 block border-t border-slate-850/40 w-full text-center pt-1.5 uppercase">
                      {day}
                    </span>
                  </div>
                );
              ...
              {data?.leaderboardPreview?.map((entry, idx) => (
                <div 
                  ...
                  className={`p-3 bg-slate-950/40 border border-slate-850 rounded-xl flex items-center justify-between gap-3 ${
                    entry.userId === data.userId ? 'border-emerald-500/30 bg-emerald-950/5' : ''
                  }`}
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <img 
                      ...
                      className="w-8 h-8 rounded-lg bg-slate-900 border border-slate-800 p-0.5 shrink-0"
                    />
                    <div className="min-w-0">
                      <span className="block text-xs font-bold text-slate-200 truncate">{entry.name}</span>
                      <span className="block text-[8px] font-bold text-emerald-400 uppercase tracking-widest leading-none mt-0.5">{entry.level}</span>
                    </div>
                  </div>
                  
                  <span className="text-xs font-black text-slate-350 shrink-0 font-mono">
                    {entry.points} XP
                  </span>
                </div>
              ))}
            ...
            {(!data?.achievementsPreview || data.achievementsPreview.length === 0) ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                {data.achievementsPreview.map(ach => (
                  <div 
                    key={ach.id} 
                    className="p-3 bg-slate-950/40 border border-slate-850 rounded-xl flex flex-col items-center justify-center text-center space-y-1.5"
                  >
                    <div className="p-2 bg-emerald-950/40 border border-emerald-900/40 text-emerald-400 rounded-lg">
                      <Award size={18} />
                    </div>
                    <span className="text-[10px] font-bold text-slate-200 truncate w-full">{ach.title}</span>
                    <span className="text-[8px] font-black text-emerald-400 bg-emerald-950/40 border border-emerald-900/35 px-1.5 py-0.5 rounded leading-none">
