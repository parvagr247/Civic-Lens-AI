...
export default function CommunityFeed() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 feed-container">
      <div>
        <h2 className="text-xl font-black text-white">Community Collaboration Feed</h2>
        <p className="text-xs text-slate-450 mt-1">Engage with fellow citizen reporters, save alerts, and follow resolving officers.</p>
      </div>
      ...
      {incidents.map((incident) => {
        ...
        return (
          <Card key={incident.id} className="feed-card bg-slate-900/30 border-slate-850 p-5 shadow-xl space-y-4">
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <img 
                  ...
                  alt="Reporter avatar" 
                  className="w-10 h-10 rounded-xl bg-slate-950 border border-slate-800 p-0.5 shrink-0"
                />
                <div>
                ...
                <button
                  ...
                  className={`flex items-center gap-1 px-3 py-1.5 border rounded-lg text-[10px] font-bold transition-all duration-200 ${
                    isFollowed 
                      ...
                      : 'bg-emerald-950/20 text-emerald-400 border-emerald-900/50 hover:bg-emerald-500 hover:text-slate-950 hover:border-transparent'
                  }`}
                >
                  {isFollowed ? (
                    <>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-350 leading-relaxed">{incident.description}</p>
            </div>

            {/* Incident Media */}
            {incident.imageUrl && (
              <div className="rounded-xl overflow-hidden border border-slate-850/80 bg-slate-950">
                <img 
            ...
            {/* Divider */}
            <div className="h-[1px] bg-slate-850/40 w-full" />

            {/* Action Bar */}
            <div className="flex items-center justify-between text-slate-450 px-1 relative">
              <button 
                onClick={() => toggleCommentsDrawer(incident.id)}
                className={`flex items-center gap-1.5 text-xs font-bold hover:text-emerald-400 transition-colors duration-200 ${
                  activeCommentIncidentId === incident.id ? 'text-emerald-400' : ''
                }`}
              >
              ...
              <div className="relative">
                ...
                {shareDropdownId === incident.id && (
                  <div className="absolute right-0 bottom-8 bg-slate-900 border border-slate-800 rounded-xl p-1.5 flex flex-col gap-1 shadow-2xl z-20 w-32 animate-in fade-in slide-in-from-bottom-2 duration-200">
                    <button 
                      onClick={() => handleShareClick(incident.id, 'copy')}
                      className="flex items-center gap-2 p-1.5 hover:bg-slate-850 rounded-lg text-[10px] font-bold text-slate-300 w-full text-left"
                    >
                      <Copy size={12} />
                      Copy Link
                    </button>
            ...
            {activeCommentIncidentId === incident.id && (
              <div className="space-y-4 pt-3 border-t border-slate-850/40 animate-in slide-in-from-top-3 duration-300">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-950/40 border border-slate-850 rounded-xl px-3.5 py-2 text-xs text-slate-200 focus:outline-none focus:border-emerald-500/50"
                  />
                  <Button
